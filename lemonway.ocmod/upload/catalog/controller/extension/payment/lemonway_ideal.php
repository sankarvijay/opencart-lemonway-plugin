...
class ControllerExtensionPaymentLemonWayIdeal extends Controller
{
    // Constants
   ...
   */
    ...
    */
    public function index()
    {
        ...
        // Load Model
        $this->load->model('extension/payment/lemonway_ideal');

        $data['link_checkout'] = $this->url->link('extension/payment/lemonway_ideal/checkout', '', true);
        $data['customerId'] = empty($this->customer->getId()) ? 0 : $this->customer->getId();

        return $this->load->view('extension/payment/lemonway_ideal', $data);
    }
    ...
    */
    public function checkout()
    {
        ...
        if (!$useCard) {// If the client use a new card => MoneyInWebInit
            ...
            $wkToken = $this->model_extension_payment_lemonway_ideal->saveWkToken($order_id, $registerCard);
            ...
            $params['returnUrl'] = $this->url->link('extension/payment/lemonway_ideal/checkoutReturn&' . $returnParams, '', true);
            $params['issuerId'] = $this->postValue('bank');
            $res = $lemonwayService->moneyInIDealInit($params);
            ...
            $query = "INSERT INTO `" . DB_PREFIX . "lemonway_wktoken` (`order_id`, `wktoken`) VALUES (" . (int)$this->db->escape($order_id) . ",'" . $this->db->escape($res->IDEALINIT->ID) . "') ON DUPLICATE KEY UPDATE wktoken = '" . $this->db->escape($res->IDEALINIT->ID) . "'";
            $this->db->query($query);

            $this->response->redirect(utf8_decode(urldecode($res->IDEALINIT->actionUrl)));
        }
    }
    ...
    */
    public function checkoutReturn()
    {
        ...
        if ($this->isGet()) { // If redirection
            ...
            $ref = end(explode(".", $reference));

            $params['transactionId'] = $ref;
            ...
            // Get order info
            $order_id = $this->model_extension_payment_lemonway_ideal->getOrderIdFromToken($ref);
            $order_info = $this->model_checkout_order->getOrder($order_id);
            ...
            $total = number_format((float)$order_info['total'], 2, '.', '');
            $res = $lemonwayService->moneyInIDealConfirm($params);
            $status = $res->TRANS->HPAY->STATUS;
            ...
            $confirm = $lemonwayService->moneyInIDealConfirm($wkToken);

            // Get order info
            $order_id = $this->model_extension_payment_lemonway_ideal->getOrderIdFromToken($wkToken);
            $order_info = $this->model_checkout_order->getOrder($order_id);
            $total = number_format((float)$order_info['total'], 2, '.', '');

            if (($response_code == "0000") && $this->checkAmount($total, $realAmount) && $this->doublecheckAmount($total, $wkToken) && $confirm->TRANS->HPAY->STATUS == "3") {
                // Success => Order status 5 : Complete
                $this->model_checkout_order->addOrderHistory($order_id, 5);
            } else {
